"""
Trains the proposed model on one regime and saves a self-contained checkpoint.
...
def main():
    ...
    torch.manual_seed(args.seed)   # seed before init, see run_gnn in experiments.py
    ...
    # Config, feature names, scaler constants and threshold all travel with the weights.
    # A bare state_dict makes the next person guess the hidden size, the feature order
    # and the normalisation, and a wrong guess gives plausible garbage, not an error.
    torch.save({
